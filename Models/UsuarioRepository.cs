...
namespace Inmobiliaria_Zarate_DoNet.Data
{
    public class UsuarioRepository
    {
        ...
        public UsuarioRepository(DbConexion db)
        {
            _db = db;
        }
        ...
        public List<Usuario> GetAll()
        {
            ...
            using (var conn = _db.CrearConexion())
            {
                ...
                using (var rd = cmd.ExecuteReader())
                {
                    ...
                    int oApellido = rd.GetOrdinal("apellido");
                    int oEmail = rd.GetOrdinal("email");
                    int oHash = rd.GetOrdinal("password_hash");
                    int oRol = rd.GetOrdinal("rol");
                    int oActivo = rd.GetOrdinal("activo");
                    int oCreado = rd.GetOrdinal("creado_en");
                    ...
                    while (rd.Read())
                    {
                        lista.Add(new Usuario
                        {
                            ...
                            Apellido = rd.GetString(oApellido),
                            Email = rd.GetString(oEmail),
                            PasswordHash = rd.GetString(oHash),
                            Rol = Enum.Parse<Rol>(rd.GetString(oRol)),
                            Activo = rd.GetBoolean(oActivo),
                            CreadoEn = rd.GetDateTime(oCreado)
                        });
                    }
                }
            }
        ...
        // CREAR (hasheando password plano)
        ...
        // ACTUALIZAR (sin tocar el password)
